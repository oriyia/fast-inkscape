...
def check(type_, self, name, event):
    ...
    if len(filtered) == 0:
        pressed.clear()
        return back_to_normal(self, event)
...
def back_to_normal(self, event):
    self.mode = normal.determine_type_action(self, event)
    pressed.clear()
...
def paste_mode(type_, self, event, char):
    ...
    if char == 'Escape':
        ...
        pressed.append(char)
        return check(type_, self, ''.join(pressed), event)
